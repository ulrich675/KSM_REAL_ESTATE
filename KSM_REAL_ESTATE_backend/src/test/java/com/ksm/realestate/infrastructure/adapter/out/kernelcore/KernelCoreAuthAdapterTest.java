...
public class KernelCoreAuthAdapterTest {
        ...
        public void testSignUp_Success() {
                ...
                StepVerifier.create(adapter.signUp(command))
                                .expectNextMatches(result -> "tok-123".equals(result.getAccessToken())
                                                && "Bearer".equals(result.getTokenType()))
                                .verifyComplete();
        }
                ...
                StepVerifier.create(adapter.signUp(command))
                                .expectErrorMatches(error -> error instanceof KernelCoreException
                                                && "CLIENT_APPLICATION_SERVICE_NOT_ALLOWED"
                                                                .equals(((KernelCoreException) error).getErrorCode()))
                                .verify();
        }
        ...
        public void testLogin_MfaRequiredReturnsNextStep() {
                ...
                StepVerifier.create(adapter.login("jdoe@ksm.com", "Passw0rd!!"))
                                .expectNextMatches(result -> "CONFIRM_MFA".equals(result.getNextStep())
                                                && "mfa-tok-1".equals(result.getMfaToken()))
                                .verifyComplete();
        }
